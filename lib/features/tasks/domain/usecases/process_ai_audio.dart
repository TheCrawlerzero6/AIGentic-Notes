...
class ProcessAiAudioUseCase {
  final IAiService aiService;
  final ITaskRepository taskRepository;

  ProcessAiAudioUseCase({
  ...
  Future<int> call({
    ...
    // Validación 1: Verificar que los bytes de audio no estén vacíos
    if (audioBytes.isEmpty) {
      throw ArgumentError('Los bytes de audio no pueden estar vacíos');
    }
    ...
    try {
      ...
      if (e.toString().contains('timeout')) {
        throw Exception('Tiempo de espera agotado al procesar el audio con IA. Intenta nuevamente.');
      } else if (e.toString().contains('quota')) {
        throw Exception('Límite de solicitudes de IA alcanzado. Espera unos minutos.');
      } else if (e.toString().contains('network')) {
        throw Exception('Error de conexión. Verifica tu conexión a internet.');
      } else {
        throw Exception('Error al procesar audio con IA: ${e.toString()}');
      }
    }

    // Validación 4: Verificar que se retornó al menos una tarea
    if (taskModels.isEmpty) {
      throw Exception('La IA no pudo extraer tareas del audio. Intenta hablar más claramente.');
    }
