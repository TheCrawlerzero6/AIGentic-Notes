...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final completedCount = taskProvider.tasks.where((t) => t.isCompleted == 1).length;
    final pendingCount = taskProvider.tasks.where((t) => t.isCompleted == 0).length;
    
    // Calcular racha (días consecutivos con tareas completadas)
    int streak = 0;
    final now = DateTime.now();
    final completedTasks = taskProvider.tasks.where((t) => t.isCompleted == 1 && t.completedAt != null).toList();
    if (completedTasks.isNotEmpty) {
      completedTasks.sort((a, b) => DateTime.parse(b.completedAt!).compareTo(DateTime.parse(a.completedAt!)));
      DateTime checkDate = DateTime(now.year, now.month, now.day);
      
      for (var task in completedTasks) {
        final completedDate = DateTime.parse(task.completedAt!);
        final taskDay = DateTime(completedDate.year, completedDate.month, completedDate.day);
        
        if (taskDay.isAtSameMomentAs(checkDate) || taskDay.isBefore(checkDate)) {
